...
class Pic_HC_trainer(SimpleTrainer):

    def __init__(self, maxiter, patience, train_seed, seed, num_compose, num_candidates):
        super(Pic_HC_trainer, self).__init__(maxiter, patience, train_seed, seed, num_compose, num_candidates)
    ...
    def mutated(self, base_candidate, phrase_lookup, use_add, delete_tracker, edit_operations, args):
        ...
        for c, candidate in enumerate(candidates):
            scores.append(self.pics_score(candidate, c + 1, args=args))
            print(scores[-1])
    ...
    def train(self, args):
        ...
        meta_file.write("\n")
        wandb.log({"original_candidate": self.original_candidate})
        ...
        while current_iteration < self.maxiter:
            ...
            # wandb.log({"step": current_iteration, "base_candidate": base_candidate})
            # wandb.log({"step": current_iteration, "base_score": base_score})
            wandb.log({"base_candidate": base_candidate})
            ...
            # wandb.log({"curr_iteration": current_iteration, "best_candidate": best_candidate})
            # wandb.log({"curr_iteration": current_iteration, "best_score": best_score})
            wandb.log({"best_candidate": best_candidate})
            ...
            print("result_candidate: ", self.result_candidate, "result_score: ", self.result_score)
            ...
            if self.patience_counter > args.patience:
                ...
                continue
        wandb.log({"result_candidate": self.result_candidate})
