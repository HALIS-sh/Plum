...
instruction = "a photo of an astronaut riding a horse on mars"
...
if 'sub' in edit_operations:
    ...
    para_tokenizer = PegasusTokenizer.from_pretrained("/home/wenhesun/.cache/huggingface/hub/models--tuner007--pegasus_paraphrase")
    para_model = PegasusForConditionalGeneration.from_pretrained("/home/wenhesun/.cache/huggingface/hub/models--tuner007--pegasus_paraphrase").to(torch_device).eval()

# prepare prompt_0 and base_prompt_pics
def initialize_prompt_0(instruction):
    ...
    for pic_count in range(1,3):
        image = sd_pipe(instruction).images[0]
...
def calc_probs(device, processor, model, prompt, images):
...
def pics_score(device, processor, model,instruction, candidate, prompt_count):
    # Use the DPMSolverMultistepScheduler (DPM-Solver++) scheduler here instead
    sd_pipe = StableDiffusionPipeline.from_pretrained("/home/wenhesun/.cache/huggingface/hub/models--stabilityai--stable-diffusion-2-1/snapshots/5cae40e6a2745ae2b01ad92ae5043f95f23644d6", torch_dtype=torch.float16)
    sd_pipe.scheduler = DPMSolverMultistepScheduler.from_config(sd_pipe.scheduler.config)
    sd_pipe = sd_pipe.to("cuda")
    ...
    for pic_count in range(1,3):
        ...
        score = score + (calc_probs(device, processor, model, instruction, pil_images)[1]*100)
    ave_score = score/5.0
...
initialize_prompt_0(instruction) 
W_candidates = [] 
W_scores = []
W_deletesets = []
...
for i in range(num_steps):
    ...
    for j in range(num_offspring):
        ...
        if offspring not in W_candidates:
            ...
            offspring_score = pics_score(device, processor, model, instruction, offspring, j)
    ...
    for i, base_candidate in enumerate(W_candidates_m):
        if mutation_prob > np.random.random():            
            ...
            for c, candidate in enumerate(candidates):
                ...
                scores.append(pics_score(device, processor, model, instruction, candidate, c))
                print(scores[-1])
